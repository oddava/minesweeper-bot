...
list-backups: ## List all available backups
	docker compose exec pgbackup find /backups -type f
...
restore: ## Restore from a backup file (args=full_path_to_file)
	docker compose exec pgbackup sh -c "PGPASSWORD=\$$POSTGRES_PASSWORD dropdb -h \$$POSTGRES_HOST -U \$$POSTGRES_USER --force --if-exists \$$POSTGRES_DB && PGPASSWORD=\$$POSTGRES_PASSWORD createdb -h \$$POSTGRES_HOST -U \$$POSTGRES_USER \$$POSTGRES_DB && PGPASSWORD=\$$POSTGRES_PASSWORD zcat $(args) | psql -h \$$POSTGRES_HOST -U \$$POSTGRES_USER -d \$$POSTGRES_DB"
